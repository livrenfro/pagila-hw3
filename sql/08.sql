...
-- all customers who rented bucket brotherhood 

other_movies AS(
    SELECT film_id, customer_id FROM rental 
    JOIN inventory USING(inventory_id) 
    WHERE customer_id IN (SELECT * FROM bucket_cust)),

choice AS(
    SELECT film_id FROM other_movies
    GROUP BY film_id
    HAVING COUNT(DISTINCT other_movies.customer_id) >= 3)

SELECT title FROM film 
JOIN choice USING(film_id)
